                    ...
                    with st.container():
                        ...
                        with col2:
                            ...
                            if article.get('content'):
                                # Show first 300 characters instead of using 'summary'
...
# Main app header (only shown for search page)
if page == "Search Articles":
    st.markdown("""
    <div class="main-header">
        <h1 class="main-title">PersonaTracker</h1>
        ...
        for date in sorted_dates:
            ...
            with st.expander(f"Articles from {date if date != 'Unknown Date' else 'Unknown Date'}", expanded=False):
                for article in articles_by_date[date]:
                    ...
                    with col1:
                        # Simplified image display with better error handling
                        if article.get('image_url'):
                            try:
                                # Clean the image URL
                                img_url = article['image_url'].strip()
                                
                                # Handle relative URLs
                                if img_url.startswith('//'):
                                    img_url = f"https:{img_url}"
                                elif img_url.startswith('/') and article.get('url'):
                                    parsed_uri = urllib.parse.urlparse(article['url'])
                                    img_url = f"{parsed_uri.scheme}://{parsed_uri.netloc}{img_url}"
                                
                                # Only try to display if it's a valid HTTP URL
                                if img_url.startswith(('http://', 'https://')):
                                    ...
                                    st.info("Image format not supported")
                                    logger.info(f"❌ Invalid image URL format: {img_url}")
                                    
                            except Exception as img_error:
                                st.info("Image unavailable")
                                logger.warning(f"❌ Error displaying image {article.get('image_url')}: {str(img_error)}")
                        else:
                            st.info("No image available")
                            logger.info("ℹ️ No image URL found in article data")
                    ...
                    with col2:
                        ...
                        # Also show a preview (first 300 characters)
